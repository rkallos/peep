defmodule Peep.Storage.StripedPreHashed do
  @moduledoc false
  ...
  @impl true
  def insert_metric(tids, %Metrics.Counter{} = metric, hash, _value, %{} = tags) do
    tid = get_tid(tids)
    key = {hash, tags}
    :ets.update_counter(tid, key, {3, 1}, {key, metric, 0})
  end

  def insert_metric(tids, %Metrics.Sum{} = metric, hash, value, %{} = tags) do
    tid = get_tid(tids)
    key = {hash, tags}
    :ets.update_counter(tid, key, {3, value}, {key, metric, 0})
  end

  def insert_metric(tids, %Metrics.LastValue{} = metric, hash, value, %{} = tags) do
    tid = get_tid(tids)
    now = System.monotonic_time()
    key = {hash, tags}
    :ets.insert(tid, {key, metric, {now, value}})
  end

  def insert_metric(tids, %Metrics.Distribution{} = metric, hash, value, %{} = tags) do
    tid = get_tid(tids)
      ...
      case :ets.lookup(tid, key) do
        ...
        [] ->
          ...
          case :ets.insert_new(tid, {key, metric, tags, new_atomics}) do
            ...
            false ->
              [{^key, ^metric, ^tags, atomics}] = :ets.lookup(tid, key)
              atomics
          end
      end
  ...
  defp add_metric({{_hash, tags}, %Metrics.Counter{} = metric, value}, acc) do
    path = [Access.key(metric, %{}), Access.key(tags, 0)]
    update_in(acc, path, &(&1 + value))
  end

  defp add_metric({{_hash, tags}, %Metrics.Sum{} = metric, value}, acc) do
    path = [Access.key(metric, %{}), Access.key(tags, 0)]
    update_in(acc, path, &(&1 + value))
  end

  defp add_metric({{_hash, tags}, %Metrics.LastValue{} = metric, {_, _} = a}, acc) do
    path = [
  ...
  defp add_metric({{_hash, _hashed_tags}, %Metrics.Distribution{} = metric, tags, atomics}, acc) do
    path = [
      Access.key(metric, %{}),
      Access.key(tags, %{})
    ]
